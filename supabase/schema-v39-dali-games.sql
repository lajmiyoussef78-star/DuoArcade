-- schema-v39-dali-games.sql — temporary shared "Dali" beta game list per duo (like favorites).
-- Plain id array: ["ttt", "sumobomb", …]. Remove later with the Dali UI.

alter table public.duos
  add column if not exists dali_games jsonb not null default '[]'::jsonb;
...
begin
  ...
  update duos set
    ...
    fix_games   = coalesce(p_patch->'fix_games', fix_games),
    dali_games  = coalesce(p_patch->'dali_games', dali_games),
    session     = case
