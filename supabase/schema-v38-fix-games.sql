-- schema-v38-fix-games.sql — shared "Needs fix" game list per duo (like favorites).
-- Stores [{ "id": "ttt", "note": "…" }, …]. Does not edit app code.

alter table public.duos
  add column if not exists fix_games jsonb not null default '[]'::jsonb;
