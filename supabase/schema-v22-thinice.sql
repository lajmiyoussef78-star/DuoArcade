-- schema-v22-thinice.sql — DuoArcade "Thin Ice": lifetime match tally.
-- Run once in the SQL Editor.

create table if not exists public.thinice_results (
...
alter table public.thinice_results enable row level security;

create policy "thinice: duo members read"
  on public.thinice_results for select
...
create or replace function public.record_thinice(p_duo_code text, p_winner text)
returns json
...
begin
  ...
  insert into thinice_results (duo_code, wins_a, wins_b)
  values (p_duo_code,
          case when p_winner = 'A' then 1 else 0 end,
          case when p_winner = 'B' then 1 else 0 end)
  on conflict (duo_code) do update set
    wins_a = thinice_results.wins_a + case when p_winner = 'A' then 1 else 0 end,
    wins_b = thinice_results.wins_b + case when p_winner = 'B' then 1 else 0 end;

  select * into r from thinice_results where duo_code = p_duo_code;
  return json_build_object('wins_a', r.wins_a, 'wins_b', r.wins_b);
end;
$$;

grant execute on function public.record_thinice(text, text) to authenticated;
