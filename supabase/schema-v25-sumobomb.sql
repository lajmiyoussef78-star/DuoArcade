-- schema-v25-sumobomb.sql — DuoArcade "Sumo Bomb": lifetime match tally.
-- Run once in the SQL Editor.

create table if not exists public.sumobomb_results (
...
alter table public.sumobomb_results enable row level security;

create policy "sumobomb: duo members read"
  on public.sumobomb_results for select
...
create or replace function public.record_sumobomb(p_duo_code text, p_winner text)
returns json
...
begin
  ...
  insert into sumobomb_results (duo_code, wins_a, wins_b)
  values (p_duo_code,
          case when p_winner = 'A' then 1 else 0 end,
          case when p_winner = 'B' then 1 else 0 end)
  on conflict (duo_code) do update set
    wins_a = sumobomb_results.wins_a + case when p_winner = 'A' then 1 else 0 end,
    wins_b = sumobomb_results.wins_b + case when p_winner = 'B' then 1 else 0 end;

  select * into r from sumobomb_results where duo_code = p_duo_code;
  return json_build_object('wins_a', r.wins_a, 'wins_b', r.wins_b);
end;
$$;

grant execute on function public.record_sumobomb(text, text) to authenticated;
