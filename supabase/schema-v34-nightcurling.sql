-- schema-v34-nightcurling.sql — DuoArcade "Night Curling": lifetime tally.
-- Run once in the SQL Editor.

create table if not exists public.nightcurling_results (
...
alter table public.nightcurling_results enable row level security;

create policy "nightcurling: duo members read"
  on public.nightcurling_results for select
...
create or replace function public.record_nightcurling(p_duo_code text, p_winner text)
returns json
...
begin
  ...
  insert into nightcurling_results (duo_code, wins_a, wins_b)
  values (p_duo_code,
          case when p_winner = 'A' then 1 else 0 end,
          case when p_winner = 'B' then 1 else 0 end)
  on conflict (duo_code) do update set
    wins_a = nightcurling_results.wins_a + case when p_winner = 'A' then 1 else 0 end,
    wins_b = nightcurling_results.wins_b + case when p_winner = 'B' then 1 else 0 end;

  select * into r from nightcurling_results where duo_code = p_duo_code;
  return json_build_object('wins_a', r.wins_a, 'wins_b', r.wins_b);
end;
$$;

grant execute on function public.record_nightcurling(text, text) to authenticated;
