-- schema-v24-wordbomb.sql — DuoArcade "Word Bomb": lifetime win tally.
-- Run once in the SQL Editor. Gameplay is realtime over a broadcast
-- channel; this table only stores wins per duo.

create table if not exists public.wordbomb_results (
...
alter table public.wordbomb_results enable row level security;

create policy "wordbomb: duo members read"
  on public.wordbomb_results for select
...
create or replace function public.record_wordbomb(p_duo_code text, p_winner text)
returns json
...
begin
  ...
  insert into wordbomb_results (duo_code, wins_a, wins_b)
  values (p_duo_code,
          case when p_winner = 'A' then 1 else 0 end,
          case when p_winner = 'B' then 1 else 0 end)
  on conflict (duo_code) do update set
    wins_a = wordbomb_results.wins_a + case when p_winner = 'A' then 1 else 0 end,
    wins_b = wordbomb_results.wins_b + case when p_winner = 'B' then 1 else 0 end;

  select * into r from wordbomb_results where duo_code = p_duo_code;
  return json_build_object('wins_a', r.wins_a, 'wins_b', r.wins_b);
end;
$$;

grant execute on function public.record_wordbomb(text, text) to authenticated;
