...
import { loadTimetable, weekChannel, fmtTime } from '../lib/timetable.js';
...
export default function WeekCard({ code }) {
  ...
  const reload = useCallback(async () => {
    try { const t = await loadTimetable(code); setEvents(t.events); } catch { /* none yet */ }
  }, [code]);
        ...
        <div className="wkc-list">
          {todays.map(ev => (
            <div className="wkc-item" key={ev.id}>
              ...
              <span className="t">{fmtTime(ev.start)}</span>
